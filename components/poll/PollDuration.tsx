...
export default function PollDuration({ pollData, updatePollData }: PollDurationProps) {
  ...
  const isSelected = (preset: (typeof durationPresets)[0]) => {
    return pollData.duration.value === preset.value && pollData.duration.unit === preset.unit
  }
    ...
    <div className="space-y-8">
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-3 mb-4">
          ...
          <h2 className="text-2xl font-bold text-yellow-metal-900">Poll Duration</h2>
        </div>
        <p className="text-yellow-metal-700">Choose how long your poll will be active</p>
      </div>

      <div className="space-y-6">
        {/* Quick Presets */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-yellow-metal-800">Quick Presets</h3>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span
                      className={`font-medium ${
                        isSelected(preset) ? "text-yellow-metal-900" : "text-yellow-metal-700"
                      }`}
                    >
                      {preset.label}
                    </span>
        ...
        {/* Custom Duration */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-yellow-metal-800">Custom Duration</h3>
          <div className="bg-white border-2 border-yellow-metal-200 rounded-xl p-6">
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <label className="block text-sm font-medium text-yellow-metal-700 mb-2">Duration Value</label>
                <input
                  type="number"
                  min="1"
                  max="52"
                  value={pollData.duration.value}
                  onChange={(e) => updateDuration("value", Number.parseInt(e.target.value) || 1)}
              ...
              <div className="flex-1">
                <label className="block text-sm font-medium text-yellow-metal-700 mb-2">Time Unit</label>
                <select
        ...
        <div className="bg-gradient-to-r from-yellow-metal-100 to-yellow-metal-200 rounded-xl p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-semibold text-yellow-metal-900">Poll Duration Summary</h4>
        ...
        <div className="space-y-4 pt-4 border-t border-yellow-metal-200">
          <h3 className="text-lg font-semibold text-yellow-metal-800">Additional Settings</h3>
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-yellow-metal-800">Send reminder notifications</span>
                <p className="text-xs text-yellow-metal-600">Notify participants before the poll closes</p>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-yellow-metal-800">Auto-archive after closing</span>
                <p className="text-xs text-yellow-metal-600">Automatically move to archived polls</p>
              </div>
            </label>
          </div>
        </div>
